			     ...
			     umode_t mode)
{
	...
	if (flags & O_CREAT) {
		/*
		 * TODO(b/312315630): replace fuse_init_security() with
		 * get_create_ext() ,and move it before sending fuse request when
		 * upstream kernel patch: "fuse: add request extension" is merged.
		 */
		err = fuse_init_security(fc, inode, dir, &entry->d_name);
		if (err) {
			flags &= ~(O_CREAT | O_EXCL | O_TRUNC);
				 ...
				 umode_t mode)
{
	...
	return create_new_entry(fm, &args, dir, entry, S_IFREG, true);
}
